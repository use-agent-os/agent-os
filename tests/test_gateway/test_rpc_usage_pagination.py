...
async def test_usage_cost_fallback_queries_agent_id_and_falls_back_to_session_agent_id() -> None:
    """When tracker has no scope or key disagrees, stored agent_id and channel must resolve."""
    rows = [
        _make_session(f"agent:main:noise{i:03d}", agent_id="main", channel="webchat", cost_usd=0.01)
        for i in range(120)
    ]
    analyst_rows = [
        _make_session("agent:main:analyst-1", agent_id="analyst", channel="slack", cost_usd=0.02),
        _make_session("agent:main:analyst-2", agent_id="analyst", channel="slack", cost_usd=0.03),
    ]
    mgr = _PaginatingSessionManager([*rows, *analyst_rows])
    # Tracker parses key "agent:main:analyst-1" as agent="main", channel="system",
    # but stored session records have agent_id="analyst" and channel="slack".
    ctx = _ctx(session_manager=mgr, usage_tracker=UsageTracker())

    payload_agent = await _handle_usage_cost({"agentId": "analyst"}, ctx)
    assert len(payload_agent["breakdown"]) == 2
    assert {r["sessionKey"] for r in payload_agent["breakdown"]} == {
        "agent:main:analyst-1",
        "agent:main:analyst-2",
    }
    assert payload_agent["totalCostUsd"] == 0.05

    payload_channel = await _handle_usage_cost({"channelType": "slack"}, ctx)
    assert len(payload_channel["breakdown"]) == 2
    assert {r["sessionKey"] for r in payload_channel["breakdown"]} == {
        "agent:main:analyst-1",
        "agent:main:analyst-2",
    }
    assert payload_channel["totalCostUsd"] == 0.05
