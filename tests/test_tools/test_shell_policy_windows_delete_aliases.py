...
The ``rm``/``ri`` patterns and the Windows list *extending* the shared one
(so ``shutdown``, ``rm -rf /``, ``mkfs``, ``dd if=`` and the fork bomb are
gated there too) landed first through #2690, which was stacked on this
branch; those tests stay here as the contract. What this branch still adds:
...
* Native ``format`` beside the already-denied ``Format-Volume``.
* A leading word boundary on ``mkfs``/``shutdown``/``reboot``/``halt``, so
  ``echo asphalt`` and ``python autoshutdown.py`` are not blocked outright.
