...
- `multi-search-engine`: `_ddg_search` returned 1 result instead of an empty list when `limit <= 0` and sent an unnecessary HTTP request to DuckDuckGo; it now returns an empty list immediately for `limit <= 0`.
