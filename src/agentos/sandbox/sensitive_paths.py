...
# ``~/.config`` at large.
#
# The directory entries come from the redaction layer's list, the same way
# the file entries above already do, so a directory added there also gates a
# shell read of the same path -- the two lists drifted once (#1138 added
# ~/.azure, ~/.config/gh, ~/.anthropic, ~/.openai and ~/.password-store here
# without adding them to redact.py, so cat of a blocked file skipped the
# assignment pass entirely; see #2621).
