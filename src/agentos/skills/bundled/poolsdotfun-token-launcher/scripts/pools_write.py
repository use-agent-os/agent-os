...
def resolve_signer(args: dict) -> dict:
    ...
    if args.get("from") and args["from"] is not True:
        return {
            "address": checksum_address(args["from"]),
            "privateKey": None,
            "simulateOnly": True,
            "signerEnv": None,
        }
...
def cmd_launch(client: RpcClient, args: dict) -> None:
    ...
    slippage_bps = opt_int(args, "slippage-bps", DEFAULT_SLIPPAGE_BPS, minimum=0, maximum=9_999)
    ...
    plan = plan_launch(
        ...
        deadline=deadline,
        dev_buy_wei=dev_buy_wei,
        dev_buy_asset=dev_buy_asset,
        slippage_bps=slippage_bps,
        allow_fallback_tick=bool(args.get("allow-fallback-tick")),
        salt=opt_str(args, "salt"),
        max_salt_attempts=max_salt_attempts,
    )
    ...
    receipt = _send(
        client, signer, PARTY_FACTORY, data, args, value=dev_buy_wei, gas_floor=LAUNCH_GAS_FLOOR
    )

    launched = decode_token_launched(receipt.get("logs") or [], PARTY_FACTORY)
    print(heading("launched"))
    pairs = [
        ("token", launched["token"] if launched else fresh["token"]),
    ...
    if launched and launched["devBuyAmountOut"]:
        pairs.append(("you received", f"{fmt_units(launched['devBuyAmountOut'], 18)} {symbol}"))
...
def _render_launch_plan(plan: dict, args: dict, signer: dict) -> None:
    ...
    pairs: list[tuple[str, str]] = [
        ...
        ("paired asset", f"{asset_label(paired)}  {paired}" + mark(not args.get("paired"))),
        ("salt", f"{plan['salt'][:12]}…{plan['salt'][-4:]}  (mined, {plan['saltAttempts']} tries)"),
        ("token address", f"{plan['token']}  (predicted)"),
        ("pool address", plan["pool"]),
        (
            "start tick",
            f"{plan['expectedStartTick']}  ({'live feed' if plan['tickLive'] else 'FALLBACK'})",
        ),
        ...
        (
            "creator",
            plan["creator"]
            + ("  (signer)" if not signer["simulateOnly"] else "  (--from, planning only)"),
        ),
        ("fee recipient", plan["feeRecipient"] + mark(not args.get("fee-recipient"))),
    ]
    if dev_wei:
        pairs += [
            ...
            (
                "you receive",
                f"~{fmt_units(plan['devBuyOut'], 18)} {plan['symbol']}  "
                f"({plan['supplyPct']:.4f}% of supply)",
            ),
            (
                "min accepted",
                f"{fmt_units(plan['devBuyMinOut'], 18)} "
                f"({plan['slippageBps'] / 100:.2f}% slippage)" + mark(not args.get("slippage-bps")),
            ),
        ]
    elif dev_asset:
        pairs += [
    ...
    pairs.append(
        (
            "deadline",
            f"+{int(args.get('deadline-secs') or DEFAULT_DEADLINE_SECS)}s"
...
# ── fees ────────────────────────────────────────────────────────────────────
def _locker_write(client: RpcClient, args: dict, function_name: str, command: str) -> None:
    ...
    if not _confirmed(args, digest):
        print("\n  Nothing was sent. To execute:")
        print(f"    python3 pools_write.py {command} {token} --broadcast --confirm {digest}")
        return
...
def cmd_set_fee_recipient(client: RpcClient, args: dict) -> None:
    ...
    if not _confirmed(args, digest):
        print("\n  Nothing was sent. To execute:")
        print(
            f"    python3 pools_write.py set-fee-recipient {token} "
...
def cmd_approve(client: RpcClient, args: dict) -> None:
    ...
    if raw_amount is not None:
        amount = _amount(raw_amount, 18, "amount")
    else:
        amount = 2**256 - 1
    current = client.read(paired, ERC20_ABI, "allowance", [signer["address"], PARTY_FACTORY])

    digest = action_hash(
        "approve",
        {
            "asset": paired,
            "spender": PARTY_FACTORY,
            "amount": amount,
    ...
    if not _confirmed(args, digest):
        parts = [
            "python3 pools_write.py approve",
            f"--paired {asset_label(paired).lower()}",
        ]
        if raw_amount is not None:
            parts.append(f"--amount {shlex.quote(str(raw_amount))}")
        parts.extend(["--broadcast", f"--confirm {digest}"])
        print("\n  Nothing was sent. To execute:")
        print("    " + " ".join(parts))
        return
