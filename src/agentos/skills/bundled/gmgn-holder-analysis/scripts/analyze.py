...
if len(sys.argv) > 1 and sys.argv[1] in ("-h", "--help"):
    print(f"Usage: {sys.argv[0]} <token> <chain> [zh|en]")
    sys.exit(0)

if len(sys.argv) < 3:
    print(
        f"Usage: {sys.argv[0]} <token> <chain> [zh|en]",
        file=sys.stderr,
    )
    sys.exit(2)
...
funded = [
    ((h.get("native_transfer") or {}).get("timestamp", 0), h)
    for h in normal
    if (h.get("native_transfer") or {}).get("timestamp", 0)
]
...
def native_usd(h):
    return int(h.get("native_balance") or 0) / NATIVE_DENOM * NATIVE_PRICE
...
def trend_str(wlist):
    buying = [
        h
        for h in wlist
        if (h.get("buy_tx_count_cur") or 0) > 0 and (h.get("sell_tx_count_cur") or 0) == 0
    ]
    selling = [
        h
        for h in wlist
        if (h.get("sell_tx_count_cur") or 0) > 0 and (h.get("buy_tx_count_cur") or 0) == 0
    ]
    both = [
        h
        for h in wlist
...
def is_active(h):
    return (h.get("buy_tx_count_cur") or 0) + (h.get("sell_tx_count_cur") or 0) > 0


def is_selling(h):
    return (h.get("sell_tx_count_cur") or 0) > 0 and (h.get("balance") or 0) >= 1


def is_buying_only(h):
    return (h.get("buy_tx_count_cur") or 0) > 0 and (h.get("sell_tx_count_cur") or 0) == 0
...
if biggest and biggest["amount_percentage"] > 0.15:
    dangers.append(
        _(
            f"最大单钱包持仓 {pct(biggest['amount_percentage']):.1f}%，筹码极度集中",
...
if creator:
    ...
    if to_addr and to_addr in top100_map:
        ...
        print(
            f"     {addr_short(to_addr)}  {_('持仓', 'hold')} {pct(target.get('amount_percentage', 0)):.2f}%  {_('标签', 'tags')}: {' '.join(t_mtags) or _('无', 'none')}"
        )
    ...
    if created_data:
        ...
        print(
            f"  {_('历史发币', 'Token history')}   {_('共', 'total')} {total_cnt}   {_('已迁移', 'migrated')} {mig_cnt}   {_('未迁移', 'unmigrated')} {nonmig_cnt}"
        )
        top3_mc = sorted(all_tokens, key=lambda t: float(t.get("market_cap") or 0), reverse=True)[
            :3
        ]
        if top3_mc:
        ...
        if ath_info and ath_info.get("ath_mc"):
            ...
            print(
                f"  {_('历史最高市值', 'All-time high MC')}: {ath_info.get('token_name', '')}({ath_info.get('token_symbol', '?')}){curr_label}  ATH {usd(float(ath_info.get('ath_mc') or 0))}"
            )
        print()
...
if related:
    ...
    if win_groups:
        ...
        if win_total >= 3:
            ...
            print(
                f"      {_('最大批次', 'Largest batch')}: {len(v)} {_('个钱包', 'wallets')}   {_('合计持仓', 'total hold')} {pct(sum(h['amount_percentage'] for h in v)):.3f}%"
            )
        else:
            print(
...
if kol:
    for h in kol:
        ...
        print(
            f"    · {name}  {_('持仓', 'hold')} {pct(h['amount_percentage']):.2f}%  {holding_status(h)}  {_('买/卖', 'buy/sell')}: {h.get('buy_tx_count_cur', 0)}/{h.get('sell_tx_count_cur', 0)}"
        )
print(
...
kol_holding = [
    h for h in kol if (h.get("sell_tx_count_cur") or 0) == 0 and (h.get("balance") or 0) >= 1
]
smart_selling = [h for h in smart if (h.get("sell_tx_count_cur") or 0) > 0]
smart_holding = [
    h for h in smart if (h.get("sell_tx_count_cur") or 0) == 0 and (h.get("balance") or 0) >= 1
]
if sig_count == 0:
    sig_summary = _(
...
sig_clusters = sorted(
    ...
    key=lambda x: -sum(h["amount_percentage"] for h in x[1]),
)[:4]
...
print(
    f"  {_('套牢盘（浮亏>20%）', 'Underwater (>20% loss)')}   {len(trapped)}   {_('持仓', 'hold')} {pct(sum(h['amount_percentage'] for h in trapped)):.2f}%  {tf_flag}"
)
