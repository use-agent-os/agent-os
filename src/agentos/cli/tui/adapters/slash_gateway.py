...
async def handle_gateway_slash_command(
    ...
    if parts := _slash_parts(cmd, "/new"):
        ...
        console.print(f"[green]Started new session{markup_escape(label)}:[/green] {session_key}")
        return True
...
async def _save_gateway_transcript_command(
    ...
    console.print(f"[green]Saved transcript:[/green] {markup_escape(target)}")
...
async def _handle_approvals_command(cmd: str, client: object | None = None) -> None:
    ...
    if client is None:
        ...
        if arg == "reset":
            ...
            return
        entries = [
            f"  [dim]{markup_escape(scope)}[/dim] "
            f"{markup_escape(s)}: {markup_escape(k)}:{markup_escape(t)}"
            if s
