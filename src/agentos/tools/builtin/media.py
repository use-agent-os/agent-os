...
def _dubbing_media_mime_type(path: Path) -> str:
    """MIME type resolver for dubbing_generate source_media.

    Dubbing accepts video containers (.mov, .mkv, .avi, .flv, .mp4, .webm)
    as well as audio formats. Shared extensions (.mp4, .webm) map to video/*
    for dubbing jobs, while _audio_mime_type continues to map them to audio/*
    for voice_clone and voice_convert.
    """
    ext = path.suffix.lstrip(".").lower()
    video_mapping = {
        "mp4": "video/mp4",
...
async def _resolve_supported_audio_file_for_tool(
    *,
    ...
    if ext not in allowed_formats:
        raise ToolError(
            f"Unsupported audio format: {ext}. Supported: {', '.join(sorted(allowed_formats))}"
        )
    loop = asyncio.get_running_loop()
